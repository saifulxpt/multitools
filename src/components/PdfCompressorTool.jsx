...
const PdfCompressorTool = () => {
  ...
  const compressPdf = async () => {
    ...
    try {
      ...
      // Ensure targetBytes isn't unrealistically small (minimum 50KB per page roughly, just to prevent infinite loops)
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        // Directly map target size or level to visual quality
        if (mode === 'target') {
        ...
        // Downscale loop: Try to compress down to target bytes per page
        ...
        // original page dimensions (unscaled)
      ...
      let finalSize = finalBytes.byteLength;
      ...
      alert('Error compressing PDF. Check if the file is encrypted or corrupted.');
    } finally {
          ...
          {!selectedFile ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div className="glass-panel" style={{ padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button className="btn-secondary" onClick={() => { setSelectedFile(null); setCompressedPdfBytes(null); setResultStats(null); }}>
                  Choose Another
                </button>
              </div>
